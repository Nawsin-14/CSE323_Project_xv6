...
main(int argc, char *argv[])
{
  ...
  getpinfo(px);
  22:	e8 d6 03 00 00       	call   3fd <getpinfo>
  printf(1,"PID\tINUSE\tRUNTIME\tNUM_RUN\tCURR_Q\n");
  27:	58                   	pop    %eax
  28:	5a                   	pop    %edx
  29:	68 b8 07 00 00       	push   $0x7b8
  2e:	6a 01                	push   $0x1
  30:	8d b3 00 01 00 00    	lea    0x100(%ebx),%esi
  36:	e8 75 04 00 00       	call   4b0 <printf>
  for (int i = 0; i < 64; i++)
  3b:	83 c4 10             	add    $0x10,%esp
  3e:	eb 07                	jmp    47 <main+0x47>
  40:	83 c3 04             	add    $0x4,%ebx
  43:	39 f3                	cmp    %esi,%ebx
  45:	74 39                	je     80 <main+0x80>
  {
    if(px->runtime[i]>0)
  47:	8b 83 00 02 00 00    	mov    0x200(%ebx),%eax
  4d:	85 c0                	test   %eax,%eax
  4f:	74 ef                	je     40 <main+0x40>
    printf(1,"%d\t%d\t%d\t%d\t%d\n",px->pid[i],px->inuse[i],px->runtime[i],px->num_run[i],px->current_queue[i]);
  51:	83 ec 04             	sub    $0x4,%esp
  54:	ff b3 00 04 00 00    	push   0x400(%ebx)
  for (int i = 0; i < 64; i++)
  5a:	83 c3 04             	add    $0x4,%ebx
    printf(1,"%d\t%d\t%d\t%d\t%d\n",px->pid[i],px->inuse[i],px->runtime[i],px->num_run[i],px->current_queue[i]);
  5d:	ff b3 fc 02 00 00    	push   0x2fc(%ebx)
  63:	50                   	push   %eax
  64:	ff 73 fc             	push   -0x4(%ebx)
  67:	ff b3 fc 00 00 00    	push   0xfc(%ebx)
  6d:	68 da 07 00 00       	push   $0x7da
  72:	6a 01                	push   $0x1
  74:	e8 37 04 00 00       	call   4b0 <printf>
  79:	83 c4 20             	add    $0x20,%esp
  for (int i = 0; i < 64; i++)
  7c:	39 f3                	cmp    %esi,%ebx
  7e:	75 c7                	jne    47 <main+0x47>
  }
  
  exit();
  80:	e8 c0 02 00 00       	call   345 <exit>
  85:	66 90                	xchg   %ax,%ax
  87:	66 90                	xchg   %ax,%ax
  89:	66 90                	xchg   %ax,%ax
  8b:	66 90                	xchg   %ax,%ax
 ...
 4b9:	8b 75 0c             	mov    0xc(%ebp),%esi
{
 ...
 630:	b8 28 00 00 00       	mov    $0x28,%eax
          s = "(null)";
 635:	bb ea 07 00 00       	mov    $0x7ea,%ebx
 ...
 641:	a1 2c 0b 00 00       	mov    0xb2c,%eax
{
 ...
 67f:	5b                   	pop    %ebx
  freep = p;
 680:	a3 2c 0b 00 00       	mov    %eax,0xb2c
}
 685:	5e                   	pop    %esi
 686:	5f                   	pop    %edi
 687:	5d                   	pop    %ebp
 ...
 696:	72 ce                	jb     666 <free+0x26>
{
 ...
 6b7:	03 53 fc             	add    -0x4(%ebx),%edx
  freep = p;
 6ba:	a3 2c 0b 00 00       	mov    %eax,0xb2c
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 6d9:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 6dc:	8b 15 2c 0b 00 00    	mov    0xb2c,%edx
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 71b:	3b 05 2c 0b 00 00    	cmp    0xb2c,%eax
 ...
 73e:	e8 fd fe ff ff       	call   640 <free>
  return freep;
 743:	8b 15 2c 0b 00 00    	mov    0xb2c,%edx
 ...
 787:	0b 00 00 
    base.s.size = 0;
 78a:	b8 30 0b 00 00       	mov    $0xb30,%eax
    base.s.ptr = freep = prevp = &base;
 78f:	c7 05 30 0b 00 00 30 	movl   $0xb30,0xb30
 796:	0b 00 00 
    base.s.size = 0;
 799:	c7 05 34 0b 00 00 00 	movl   $0x0,0xb34
 7a0:	00 00 00 
    if(p->s.size >= nunits){
